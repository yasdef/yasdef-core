...
### Requirement: Feature-sync metadata written during routing omits selection_mode
When the orchestrator writes `feature_sync.yaml` after step routing (both fresh selection and resume reuse), the written file SHALL NOT contain a `selection_mode` field.

#### Scenario: Fresh routing writes feature_sync.yaml without selection_mode
- **WHEN** the orchestrator completes step routing for a fresh (non-resume) invocation and calls `write_feature_sync_metadata`
- **THEN** the resulting `feature_sync.yaml` does not contain a `selection_mode` key

#### Scenario: Resume routing rewrites feature_sync.yaml without selection_mode
- **WHEN** the orchestrator completes resume reuse validation and calls `write_feature_sync_metadata`
- **THEN** the resulting `feature_sync.yaml` does not contain a `selection_mode` key
