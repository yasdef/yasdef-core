...
- **WHEN** the design readiness helper checks a design artifact that exists, contains `## Goal`, `## In Scope`, and `## Out of Scope`, and has no unresolved first-feature bootstrap decision
...
#### Scenario: Bootstrap-required design without resolved scaffold handoff returns failure
- **WHEN** the design artifact records `Bootstrap required: yes` but does not contain resolved blueprint-backed scaffold handoff data or explicit user stack direction
- **THEN** the helper exits non-zero
