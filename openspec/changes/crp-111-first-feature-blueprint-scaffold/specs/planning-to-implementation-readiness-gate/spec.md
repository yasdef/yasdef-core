...
- **WHEN** the readiness helper checks a step whose step plan is missing, whose required planning sections are missing, or whose `overmind/implementation_plan.md` `Plan and discuss the step` bullet is missing or unchecked
- **THEN** the helper exits non-zero and reports the failing condition

#### Scenario: Bootstrap-required planning without scaffold work returns failure
- **WHEN** the linked design artifact records `Bootstrap required: yes` and the step plan omits mandatory scaffold creation work derived from the design handoff
- **THEN** the helper exits non-zero and reports the missing scaffold planning contract
