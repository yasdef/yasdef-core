...
Replace `_configure_design_only` with a complete five-row echo configuration. Thirteen of its twenty call sites exit during discovery or validation before phase execution and can retain their current product assertions unchanged. Rework the seven successful partial-pipeline cases into focused unit/application coverage or supply complete workflow prerequisites where end-to-end execution is what the test actually verifies. Keep one end-to-end invalid-configuration case to verify the CLI-facing diagnostic and no-side-effect boundary.
...
Alternative considered: delete or migrate all twenty integration call sites. Rejected because thirteen already test failures that occur before phase execution and become valid unchanged once their helper supplies a complete configuration.
