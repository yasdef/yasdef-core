...
For each step, Implementation and User Review phases MUST use only `step_plans/step-<N>.md` `## Plan (ordered)` bullets as executable state.
...
- **THEN** it reads `## Plan (ordered)` from `step_plans/step-<N>.md` and does not use `implementation_plan.md` target bullets as phase state
...
- **THEN** it validates `## Plan (ordered)` bullet state from `step_plans/step-<N>.md` only
...
- **THEN** the bullet remains `[ ]` in `step_plans/step-<N>.md`
...
- **THEN** that bullet is updated to `[x]` in `step_plans/step-<N>.md`
