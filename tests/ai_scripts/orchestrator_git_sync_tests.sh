...
EOF
  ...
  assert_file_contains "$repo_dir/.asdlc_worker/overmind/implementation_plan.md" "$worker_uuid"
}
...
EOF
  ...
  assert_contains "$out" "Failed to sync the bound ASDLC project repo before default-mode feature discovery and artifact mirroring: $source_dir"
  assert_contains "$out" "Resolve the ASDLC repo rebase conflict or dirty state in $source_dir and rerun orchestrator."
  if [[ -e "$repo_dir/.asdlc_worker/feature_sync.yaml" ]]; then
    echo "Assertion failed: feature_sync.yaml should not be created on inbound sync failure" >&2
    exit 1
  fi
}
...
test_outbound_copy_failure_stops_before_post_review_noninteractive() {
  local repo_dir="$TMP_ROOT/repo-outbound-copy-failure"
  local source_dir="$TMP_ROOT/source-outbound-copy-failure"
  local project_id="project-outbound-copy-failure"
  local worker_uuid="10000000-0000-0000-0000-000000000005"
  local out=""
  local status=0

  mkdir -p "$repo_dir"
  setup_worker_repo "$repo_dir" "echo \"ai_audit\"; printf '\n# copy-failure\n' >> .asdlc_worker/overmind/implementation_plan.md; chmod 0444 '$source_dir/feature-a/implementation_plan.md'" "echo \"post_review\""
  init_project_repo "$source_dir" "$project_id" "$worker_uuid"
  create_feature "$source_dir" "feature-a" "### Step 1.1 Copy failure step
#### Assigned: $worker_uuid
- [ ] Plan and discuss the step (SP=1)
"
  write_binding "$repo_dir" "$source_dir" "$project_id" "$worker_uuid"
  ...
  assert_nonzero_status "$status"
  assert_contains "$out" "Global implementation-plan sync failed while copying the worker runtime implementation plan to $source_dir/feature-a/implementation_plan.md."
...
test_outbound_sync_fails_when_selected_source_plan_is_already_staged() {
  local repo_dir="$TMP_ROOT/repo-outbound-prestaged-plan"
  local source_dir="$TMP_ROOT/source-outbound-prestaged-plan"
  local project_id="project-outbound-prestaged-plan"
  ...
  mkdir -p "$repo_dir"
  init_project_repo "$source_dir" "$project_id" "$worker_uuid"
  setup_worker_repo "$repo_dir" "echo \"ai_audit\"; printf '\n# staged-sync-content\n' >> .asdlc_worker/overmind/implementation_plan.md; printf '\n# already-staged-local-edit\n' >> '$source_dir/feature-a/implementation_plan.md'; (cd '$source_dir' && git add feature-a/implementation_plan.md)" "echo \"post_review\""
  create_feature "$source_dir" "feature-a" "### Step 1.1 Pre-staged step
#### Assigned: $worker_uuid
- [ ] Plan and discuss the step (SP=1)
"
  write_binding "$repo_dir" "$source_dir" "$project_id" "$worker_uuid"
  ...
  assert_nonzero_status "$status"
  assert_contains "$out" "Global implementation-plan sync failed because the selected ASDLC feature plan already has local changes: $source_dir/feature-a/implementation_plan.md"
  assert_contains "$out" "Clean, commit, or stash the local changes in $source_dir before rerunning orchestrator."
  assert_contains "$out" "1. retry"
  assert_contains "$out" "2. finish"
  assert_not_contains "$out" "post_review"
}
...
"
  clone_remote_editor "$source_dir" "$editor_dir"
  setup_worker_repo "$repo_dir" "echo \"ai_audit\"; cat > .asdlc_worker/overmind/implementation_plan.md <<'PLAN'
...
EOF
  ...
  (
    ...
    git commit -qm "track runtime mirror"
  )
  printf '\n# retry-success\n' >>"$repo_dir/.asdlc_worker/overmind/implementation_plan.md"
...
EOF
  ...
  printf '\n# finish-skip\n' >>"$repo_dir/.asdlc_worker/overmind/implementation_plan.md"
  chmod 0444 "$source_dir/feature-a/implementation_plan.md"

  out="$(run_orchestrator_with_tty_input "$repo_dir" $'2\n')"
  assert_contains "$out" "Global implementation-plan sync failed while copying the worker runtime implementation plan"
  assert_contains "$out" "orchestrator: skipping global implementation-plan sync for step '1.1' and continuing to post_review."
  assert_contains "$out" "post_review"
  assert_not_contains "$(cat "$source_dir/feature-a/implementation_plan.md")" "# finish-skip"
}

test_resume_fails_with_branch_sync_message_when_runtime_mirror_files_are_missing() {
  local repo_dir="$TMP_ROOT/repo-resume-missing-runtime-mirror"
  local source_dir="$TMP_ROOT/source-resume-missing-runtime-mirror"
  local project_id="project-resume-missing-runtime-mirror"
...
"
  ...
  (
    cd "$repo_dir"
    git checkout -q -b broken-resume-branch
    rm -f .asdlc_worker/overmind/implementation_plan.md .asdlc_worker/overmind/reqirements_ears.md
  )
  ...
  assert_nonzero_status "$status"
  assert_contains "$out" "Cannot resume on branch 'broken-resume-branch' because .asdlc_worker/overmind/implementation_plan.md and .asdlc_worker/overmind/reqirements_ears.md are not both present there."
  assert_contains "$out" "Sync 'broken-resume-branch' with 'overmind' branch to obtain these files and retry."
}
