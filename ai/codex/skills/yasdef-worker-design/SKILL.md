...
description: Run the YASDEF ASDLC worker design phase for an assigned step. Use when the orchestrator or user asks to create, update, validate, or finish a step design artifact under .asdlc_worker/step_designs before planning.
metadata:
...
1. Run the context builder:
   ```bash
   uv run python .codex/skills/yasdef-worker-design/scripts/build_design_context.py --step <step> --design-out <design-file> --plan <runtime-plan> --ears <runtime-ears>
   ```
...
5. Before finishing, run:
   ```bash
   uv run python .codex/skills/yasdef-worker-design/scripts/check_design_readiness.py <design-file>
   ```
