...
class UserReviewPhase(Phase):
        ...
        if readiness is not None:
            self.ctx.process.run(
                [
                    ...
                    str(readiness),
                    "--step",
                    self.ctx.feature.step,
                    "--step-plan",
                    str(self.step_plan_path()),
                ]
            )
        BranchManager(self.ctx.git, self.ctx.output).ensure_user_review_branch(
    ...
    def branch_name(self) -> str:
        return f"step-{self.ctx.feature.step}-{self.ctx.feature.feature_id}-user-review"
